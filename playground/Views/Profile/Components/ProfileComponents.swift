...
struct NutritionGoalCard: View {
    ...
    let unit: String
    let progress: Double
    var recommendedDifference: Int? = nil
    let onTap: () -> Void
            ...
            HStack(spacing: 16) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    HStack(spacing: 8) {
                        ...
                        // Show difference from recommended if available
                        if let diff = recommendedDifference, diff != 0 {
                            RecommendedDifferenceBadge(difference: diff, unit: unit)
                        }
                    }
                }
...
/// A pill showing "Custom" (orange) or "Auto" (green) goal status
struct GoalStatusBadge: View {
    ...
    private var statusColor: Color {
        isAutoGenerated ? .green : .orange
    }
    
    private var statusText: String {
        isAutoGenerated ? "Auto" : "Custom"
    }
}

// MARK: - Goal Change Indicator

/// Shows an up/down arrow with percentage change
struct GoalChangeIndicator: View {
    let changePercentage: Double
    var showPercentage: Bool = true
    
    var body: some View {
        if abs(changePercentage) > 0.5 { // Only show if change is meaningful
            HStack(spacing: 2) {
                Image(systemName: changePercentage > 0 ? "arrow.up" : "arrow.down")
                    .font(.system(size: 10, weight: .bold))
                
                if showPercentage {
                    Text("\(Int(abs(changePercentage)))%")
                        .font(.caption2)
                        .fontWeight(.medium)
                }
            }
...
/// Shows when goals were last modified
...
// MARK: - Recommended Difference Badge

/// Shows "+50 cal" or "-10g" difference from recommended
struct RecommendedDifferenceBadge: View {
    let difference: Int
    let unit: String
    ...
    var body: some View {
        if difference != 0 {
            Text(formattedDifference)
                ...
                .padding(.horizontal, 6)
                .padding(.vertical, 2)
                .background(differenceColor.opacity(0.12))
                .cornerRadius(4)
        }
    }
    ...
    private var differenceColor: Color {
        // For calories: higher than recommended is orange, lower is blue
        // This is neutral - neither is necessarily bad
        difference > 0 ? .orange : .blue
    }
}

// MARK: - Goal Summary Header

/// Header section for nutrition goals showing status and last updated
struct GoalSummaryHeader: View {
    let isAutoGenerated: Bool
    ...
    var body: some View {
        HStack {
            // Status badge
            GoalStatusBadge(isAutoGenerated: isAutoGenerated)
...
// MARK: - Macro Difference Row

/// Shows a macro with its recommended value and difference
struct MacroDifferenceRow: View {
    let title: String
    ...
    var body: some View {
        HStack {
            ...
            VStack(alignment: .trailing, spacing: 2) {
                ...
                if abs(currentValue - recommendedValue) > 1 {
                    HStack(spacing: 2) {
                        Text("Rec: \(Int(recommendedValue))\(unit)")
                            .font(.caption2)
                            .foregroundColor(.secondary)
                        
                        let diff = Int(currentValue - recommendedValue)
